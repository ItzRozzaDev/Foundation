...
import com.itzrozzadev.fo.Common.Stringer;
import com.itzrozzadev.fo.bungee.BungeeAction;
...
public final class BungeeUtil {
	 ...
	 */
	@SafeVarargs
	public static <T> void tellBungee(final BungeeAction action, final T... data) {
		...
		tellBungee(bungee.getChannel(), action, data);
	}
	 ...
	 * @param datas   The data being sent
	 */
	@SafeVarargs
	public static <T> void tellBungee(final String channel, final BungeeAction action, final T... datas) {
	 ...
	 * @param datas  The data being sent
	 */
	public static void tellNative(final Player sender, final Object... datas) {
		final ByteArrayDataOutput out = ByteStreams.newDataOutput();
	 ...
	 * @param serverName The sever the player is being sent to
	 */
	 ...
	 * through which the message is sent to
	 *
	 * @return - Who the message get sends to
	 */
	...
	/**
	 * Ensures the correct order is being read as the given {@link BungeeAction}
	 * specifies in its {@link BungeeAction#getContent()} getter.
	 * <p>
	 * This also ensures the correct data type is being read (both primitives and wrappers
	 * are supported).
	 *
	 * @param actionHead
	 * @param action
	 ...
	 * @param datas
	 */
	private static void moveHead(final int actionHead, final BungeeAction action, final Class<?> typeOf, final Object[] datas) {
		Valid.checkNotNull(action, "Action not set!");

		final Class<?>[] content = action.getContent();
		Valid.checkBoolean(actionHead < content.length, "Head out of bounds! Max data size for " + action.name() + " is " + content.length + "! . Data length: " + datas.length + " data: " + Common.join(datas));
	}
}

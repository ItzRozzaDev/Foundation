...
public final class MenuListener implements Listener {
	...
	public void onMenuClick(final InventoryClickEvent event) {
		...
		if (menu != null) {
			...
			if (action.toString().contains("PICKUP") || action.toString().contains("PLACE") || action.toString().equals("SWAP_WITH_CURSOR") || action == InventoryAction.CLONE_STACK || clickType.isShiftClick()) {
				if (whereClicked == MenuClickLocation.MENU)
					try {
						final Button button = menu.getButton(slotItem);

						if (button != null)
							menu.onButtonClick(player, event.getSlot(), action, clickType, button);
						else
							menu.onMenuClick(player, event.getSlot(), action, clickType, cursor, slotItem, !allowed);

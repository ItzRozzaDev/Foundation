...
import com.itzrozzadev.fo.model.SimpleEnchant;
...
public class ItemCreator {
	 ...
	 * @return
	 */
	public ItemStack makeSkull(final String owner) {
		...
		if (tags != null)
			if (MinecraftVersion.atLeast(V.v1_8))
				for (final Entry<String, String> entry : tags.entrySet())
					is = CompMetadata.setMetadataItem(is, entry.getKey(), entry.getValue());
	...
	/**
	 * Make an custom skull with a players uuid, custom title and lore
	 *
	 * @param uuid
	 * @param name
	 * @param lore
	 * @return
	 */
	public static ItemStack ofSkull(final UUID uuid, final String name, final String... lore) {
		final List<String> lores = Arrays.asList(lore);
		return ItemCreator.of(SkullCreator.itemFromUuid(uuid)).name(name).lores(lores).build().make();
	}

	/**
	 * Make an custom skull with a base64 String, custom title and lore
	 *
	 * @param base64
	 * @param name
	 * @param lore
	 * @return
	 */
	public static ItemStack ofSkull(final String base64, final String name, final String... lore) {
		final List<String> lores = Arrays.asList(lore);
		return ItemCreator.of(SkullCreator.itemFromBase64(base64)).name(name).lores(lores).build().make();
	}

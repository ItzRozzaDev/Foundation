...
import com.itzrozzadev.fo.ChatUtil;
...
public final class ChatPaginator {
	...
	@Setter
	private ChatColor arrowColor = ChatColor.GRAY;
	 ...
	 * @param color to use
	 */
	public ChatPaginator(final ChatColor themeColor) {
	 ...
	 * @param linesPerPage
	 */
	public ChatPaginator(final int linesPerPage) {
	...
	public ChatPaginator setFoundationHeader(final String title) {
		return this.setHeader("&r", this.themeColor + "&m" + ChatUtil.center("&r" + this.themeColor + " " + title + this.themeColor + " &m", '-', 150), "&r");
	}
	 ...
	 * @return
	 */
	public ChatPaginator setPages(final Collection<SimpleComponent> components) {
		this.pages.clear();

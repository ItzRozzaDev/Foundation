...
public class SpigotUpdater implements Runnable {
	...
	public void run() {
		...
		if (cantUpdateFrom(currentVersion))
			return;
	...
	public boolean runJoinEvent() {
		...
		try {
			final HttpURLConnection connection = (HttpURLConnection) new URL("https://api.spigotmc.org/legacy/update.php?resource=" + resourceId).openConnection();
			connection.setRequestMethod("GET");
	 ...
	 * @return
	 */
	protected boolean cantUpdateFrom(final String currentVersion) {

...
import com.itzrozzadev.fo.ReflectionUtil.ReflectionException;
...
import com.itzrozzadev.fo.model.UUIDToNameConverter;
import com.itzrozzadev.fo.plugin.SimplePlugin;
import com.itzrozzadev.fo.remain.internal.BossBarInternals;
import com.itzrozzadev.fo.remain.internal.ChatInternals;
import com.itzrozzadev.fo.remain.internal.ParticleInternals;
import com.itzrozzadev.fo.remain.nbt.NBTInternals;
import com.itzrozzadev.fo.settings.SimpleYaml;
...
import static com.itzrozzadev.fo.ReflectionUtil.getNMSClass;
import static com.itzrozzadev.fo.ReflectionUtil.getOBCClass;
...
public final class Remain {
	...
	public static void sendTitle(final Player player, final String title) {
		sendTitle(player, 20, 3 * 20, 20, title, "");
	}
	...
	public static void sendTitle(final Player player, final int fadeIn, final int stay, final int fadeOut, final String title) {
		...
		if (title != null)
			try {
				...
				if (MinecraftVersion.atLeast(V.v1_8)) {
					...
					final Constructor<?> openWindow = ReflectionUtil.getConstructor(getNMSClass(MinecraftVersion.atLeast(V.v1_7) ? "PacketPlayOutOpenWindow" : "Packet100OpenWindow", "N/A"), int.class, int.class, String.class, int.class, boolean.class);
	...
	public static OfflinePlayer getOfflinePlayerByName(final String name) {
	...
	public static String getServerName() {
		Valid.checkBoolean(Remain.isServerNameChanged(), "Please configure your 'server-name' in server.properties according to https://i.imgur.com/kecUHkJ.png before using BungeeCord features");
	...
	public static boolean isServerNameChanged() {
		return !"see https://i.imgur.com/kecUHkJ.png to configure".contains(serverName) && !"undefined".equals(serverName) && !"Unknown Server".equals(serverName);
	}

...
public class ChatInternals {
		...
		if (MinecraftVersion.newerThan(V.v1_6) && MinecraftVersion.olderThan(V.v1_12))
			try {
				...
				if (MinecraftVersion.olderThan(V.v1_8))
					...
					throw new ReflectionException("Error initiating Chat/Title/ActionBAR API (incompatible Craftbukkit? - " + Bukkit.getVersion() + " / " + Bukkit.getBukkitVersion() + " / " + MinecraftVersion.getServerVersion() + ")", ex);
				}
			}
	}
	...
	public static void sendTitleLegacy(final Player player, final int fadeIn, final int stay, final int fadeOut, final String title, final String subtitle) {
		...
		try {
			...
			throw new ReflectionException("Error sending title to: " + player.getName() + ", title: " + title + ", subtitle: " + subtitle, ex);
		}
	}
